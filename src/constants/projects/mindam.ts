...
export const mindam: Project = {
  ...
  impact:
    ...
    '공공 유사 민원 API에서 **최대 100건**을 조회하고 임베딩 기반 rerank를 적용해 분류 보강 컨텍스트 구축\n' +
    '제보 생성 전 AI 초안 단계에서 카테고리·위험도·담당 기관·부서·임베딩을 한 번에 생성하는 파이프라인 구현\n' +
    '제보 임베딩과 위치 기반 중복 후보 탐색으로 유사 제보를 하나의 이슈 그룹으로 자동 병합하는 구조 설계\n' +
    '챗봇에서 주변 제보와 내 제보를 백엔드가 검색하고 AI가 근거 기반으로 답변하는 2-step RAG 흐름 구현',
  ...
  details: {
    ...
    retrospective: [
      'RAG는 많이 검색하는 것보다 **현재 입력과 맞는 근거를 선별하는 과정**이 더 중요하다는 점을 경험',
      '외부 API는 품질 향상 요소로 활용하되 핵심 제보 플로우를 막지 않도록 **실패 격리와 graceful fallback**을 설계하는 관점 확보',
      '백엔드 도메인과 AI 파이프라인을 함께 다루며, LLM 결과를 서비스 데이터 모델과 트랜잭션 흐름에 안전하게 연결하는 역량 강화',
    ],
  },
};
